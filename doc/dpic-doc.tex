...
\subsection{Nul and multiply labeled objects}
  Gpic accepts the following code:
    \ntt{A:}
    \ntt{B: C: Here}
    \ntt{D: E: box}\\
which shows a label without an object and multiple labels for positions
and objects.  Dpic admits only unique labels but allows the first line
followed by a position or object because line ends are ignored after
the colon.
